    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-surface-800 sticky top-0 bg-surface-900">
          <p className="font-bold text-sm text-zinc-100">Elegir dia</p>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-100 text-xl leading-none">×</button>
        </div>

        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <button type="button" onClick={() => setCursor(addMonths(cursor, -1))} className="px-2 py-1 rounded hover:bg-surface-800 text-zinc-300">
              ‹
            </button>
...
function DayStrip({ selectedDay, onChange, onOpenCalendar }) {
  ...
  return (
    <div className="sticky top-0 z-20 -mx-3 bg-surface-950/95 px-3 pb-3 pt-2 backdrop-blur border-b border-surface-900">
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={() => onChange(dayKey(subDays(selectedDate, 1)))}
          className="h-10 w-9 shrink-0 rounded-lg bg-surface-900 border border-surface-800 text-zinc-300 hover:bg-surface-800"
          ...
          {days.map((date) => {
            ...
            return (
              <button
                ...
                className={`min-w-[3.25rem] rounded-xl border px-2 py-1.5 text-center transition-colors ${
                  selected
                    ? 'bg-primary text-white border-primary'
                    : 'bg-surface-900 text-zinc-300 border-surface-800 hover:bg-surface-800'
                }`}
              >
                <p className="text-[10px] uppercase leading-none opacity-75">
                  {today ? 'Hoy' : format(date, 'EEE', { locale: es })}
                </p>
                <p className="mt-1 text-base font-extrabold leading-none">{format(date, 'd')}</p>
              </button>
            )
          })}
        </div>
        ...
        <button
          ...
          className="h-10 w-10 shrink-0 rounded-lg bg-surface-900 border border-surface-800 text-zinc-300 hover:bg-surface-800"
...
export default function Fixture() {
  ...
  const dayMatches = useMemo(() => (
    partidos.filter((p) => {
      ...
      return dayKey(toZonedTime(new Date(p.scheduled_at), TZ)) === selectedDay
    })
  ...
  const leagues = useMemo(() => {
    ...
    for (const p of filtrados) {
      const key = p.league_id ?? 'sin-liga'
      if (!out[key]) {
        out[key] = {
          id: key,
      ...
      out[key].partidos.push(p)
    }
    return Object.values(out).sort((a, b) => a.nombre.localeCompare(b.nombre))
  }, [filtrados])
    ...
    <div className="flex flex-col">
      <div className="px-3 py-3 space-y-3 pb-28">
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            type="button"
            onClick={() => setSelectedDay(dayKey(zonedNow()))}
        ...
        {!isLoading && filtrados.length === 0 && (
          <div className="text-center py-16 text-zinc-500">
            ...
            <p className="font-medium text-sm">No hay partidos para este dia</p>
            <p className="text-xs text-zinc-600 mt-1">Probá moverte a otro dia del calendario</p>
          </div>
        )}

        {leagues.map((liga) => (
          <section key={liga.id} className="bg-surface-900 rounded-xl border border-surface-800 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-surface-800 bg-surface-800/40">
              ...
              <p className="font-bold text-xs truncate flex-1 text-zinc-100">{liga.nombre}</p>
              {liga.genero && (
