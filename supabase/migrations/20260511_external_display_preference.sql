-- VMScore: resolver visualmente conflictos entre fixture oficial y Copa Facil.

alter table public.external_match_archive
  add column if not exists preferred_display boolean not null default false;

drop view if exists public.v_external_matches_public;

create view public.v_external_matches_public as
select
  a.id as archive_id,
  a.source_id,
  a.external_match_id,
