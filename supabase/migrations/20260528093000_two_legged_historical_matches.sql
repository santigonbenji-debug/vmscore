-- VMScore: marcar competencias ida/vuelta y partidos historicos cargados directo.

begin;

alter table public.leagues
  add column if not exists leg_mode text not null default 'single';

alter table public.leagues
  drop constraint if exists leagues_leg_mode_check;

alter table public.leagues
  add constraint leagues_leg_mode_check
  check (leg_mode in ('single', 'two_legged'));
